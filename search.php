...
<div class="pt-section__inner">
  ...
  <div class="pt-no-results-wrap">
    ...
    <form class="pt-search-form" role="search" method="get" action="<?php echo esc_url( home_url( '/' ) ); ?>">
      <span class="pt-search-form__prompt" aria-hidden="true">$</span>
      <label for="pt-search-no-results" class="screen-reader-text"><?php esc_html_e( 'Rechercher', 'painteau' ); ?></label>
      <input id="pt-search-no-results" type="search" name="s" placeholder="<?php esc_attr_e( 'grep -r …', 'painteau' ); ?>" value="<?php echo esc_attr( get_search_query() ); ?>">
